...
class SuperHelperGit:
    ...
    # TODO think about the data strcuture every super_helper method gets!
    def configure(self, scm_path: bytes) -> None:
...
def do_unpack_for_update(superx, paths, cache_abspath: bytes, url: str, revision: Optional[str], object_id: bytes) -> None:
    ...
    do_unpack_update_metadata(paths, url, revision, object_id)

    with cwd(paths.super_abspath):
        superx.helper.add([paths.metadata_abspath])
...
def cmd_update(args, parser):
    ...
    print("Updating subproject '%s' from URL '%s' to revision '%s'..." %
          (paths.cwd_to_sub_relpath.decode("utf8"), url, cache_helper.get_revision_as_str(revision)),
          end="")
    sys.stdout.flush()
    ...
    cache_relpath = paths.cwd_to_sub_relpath + b"-tmp"
    ...
    # subpatch unpack
    do_unpack_for_update(superx, paths, cache_abspath, url, revision, object_id)
...
def do_unpack_for_add(superx, paths, cache_relpath: bytes, url: str, revision: Optional[str], object_id: bytes) -> None:
    # HACK for now
    # Check assumptions: We have just created the files ourselves
    assert os.path.exists(paths.cwd_to_sub_relpath)
    assert os.path.exists(paths.metadata_abspath)
    # TODO always check that for subprojects
    assert not os.path.exists(join(cache_relpath, b".subproject"))
    os.remove(paths.metadata_abspath)
    os.rmdir(paths.cwd_to_sub_relpath)
    os.rename(cache_relpath, paths.cwd_to_sub_relpath)
...
def cmd_add(args, parser):
    ...
    if os.path.exists(paths.cwd_to_sub_relpath):
        ...
        raise AppException(ErrorCode.CUSTOM,
                           "Directory '%s' alreay exists. Cannot add subproject!" % (paths.cwd_to_sub_relpath.decode("utf8"),))

    if os.path.exists(paths.metadata_abspath):
        ...
        raise AppException(ErrorCode.CUSTOM,
                           "File '%s' alreay exists. Cannot add subproject!" % (paths.metadata_abspath.decode("utf8"),))

    os.makedirs(paths.subproject_abspath)
    with open(paths.metadata_abspath, "bw") as f:
        f.write(b"")
    with cwd(paths.subproject_abspath):
        # TODO use helper!
        git_add([b".subproject"])

    config_add_subproject(paths.config_abspath, paths.super_to_sub_relpath)
    with cwd(paths.super_abspath):
        # TODO use helper!
        git_add([b".subpatch"])
    ...
    try:
        ...
        do_unpack_for_add(superx, paths, cache_relpath, url, revision, object_id)
...
class Paths:
    ...
    # TODO split path into paths for superproject and paths for subproject
...
# TODO refactor function order!
def is_inside_subproject_and_return_path(config: Config, paths: Paths) -> Optional[bytes]:
    for relpath in config.subprojects:
        if paths.super_to_sub_relpath.startswith(relpath):
            return relpath
    return None
...
def do_paths(super_abspath: bytes, cwd_to_sub_relpath: bytes) -> Paths:
    ...
    subproject_abspath = join(super_abspath, super_to_sub_relpath)

    metadata_abspath = join(subproject_abspath, b".subproject")

    patches_abspath = join(subproject_abspath, b"patches")

    return Paths(super_abspath, config_abspath, super_to_cwd_relpath, super_to_sub_relpath, cwd_to_sub_relpath, sub_name, subproject_abspath, metadata_abspath, patches_abspath)
...
def do_unpack_update_metadata(paths: Paths, url: str, revision: Optional[str], object_id: bytes) -> None:
    try:
        with open(paths.metadata_abspath, "br") as f:
    ...
    metadata_lines = config_set_key_value2(metadata_lines, b"upstream", b"object-id", object_id)
...
def do_pop_push_update_metadata(paths: Paths, applied_index: int) -> None:
    try:
        with open(paths.metadata_abspath, "br") as f:
            metadata_lines = config_parse2(split_with_ts_bytes(f.read()))
    except FileNotFoundError:
...
def do_pop_update_metadata_drop(paths: Paths) -> None:
    try:
        with open(paths.metadata_abspath, "br") as f:
            metadata_lines = config_parse2(split_with_ts_bytes(f.read()))
    except FileNotFoundError:
...
def is_cwd_toplevel_directory(paths: Paths) -> bool:
...
def checks_for_cmds_apply_pop_push(args) -> tuple[Any, Paths]:
    data = find_superproject()
    checked_data = check_superproject_data(data)
    superx = check_and_get_superproject_from_checked_data(checked_data)
    ensure_superproject_is_configured(superx)
    ...
    # TODO split Paths object into superproject and subproject parts!
    paths = do_paths(superx.path, b"")
    config = read_config(paths.config_abspath)

    # NOTE: This function can be used to select the subproject based on cwd!
    # TODO Refactor!
    super_to_sub_relpath = is_inside_subproject_and_return_path(config, paths)
    ...
    if len(super_to_sub_relpath) == len(paths.super_to_cwd_relpath):
        assert super_to_sub_relpath == paths.super_to_cwd_relpath
        pass  # It's the same directory
    else:
        assert len(super_to_sub_relpath) < len(paths.super_to_cwd_relpath)
        assert paths.super_to_cwd_relpath.startswith(super_to_sub_relpath)
        #  l = len(super_to_sub_relpath)
        #  p = paths.super_to_cwd_relpath
        #  assert p[l] == ord("/")
...
def cmd_apply(args, parser):
    ...
    _, paths = checks_for_cmds_apply_pop_push(args)

    metadata = read_metadata(paths.metadata_abspath)
    patches_dim = read_patches_dim(paths, metadata)
    ...
    git_args = ["git", "apply", "--index", "--directory=%s" % (paths.super_to_sub_relpath.decode("utf8"),), args.path]
    p = Popen(git_args)
    p.communicate()
    if p.returncode != 0:
    ...
    if not os.path.exists(paths.patches_abspath):
        os.makedirs(paths.patches_abspath)

    import shutil
    shutil.copy(args.path.encode("utf8"), paths.patches_abspath)
    super_to_patch_relpath = join(paths.super_to_sub_relpath, b"patches", patch_filename)

    with cwd(paths.super_abspath):
        # TODO use helper!
        git_add([super_to_patch_relpath])

    if not args.quiet:
        # TODO The output convetion is wrong here. It should be relative and
        # the path to the subrpoject is not relative to the cwd here!
        # TODO Having the subrpoject name is kind of reduntant, because the cwd
        # is already inside the subproject
        print("Applied patch '%s' to subproject '%s' successfully!" % (args.path, paths.super_to_sub_relpath.decode("utf8")))

    # TODO print commands for comitting

    return 0


def cmd_pop(args, parser):
    superx, paths = checks_for_cmds_apply_pop_push(args)

    metadata = read_metadata(join(paths.super_abspath, paths.super_to_sub_relpath, b".subproject"))
    patches_dim = read_patches_dim(paths, metadata)
    ...
    patch_abspath = join(paths.patches_abspath, patch_filename)

    p = Popen(["git", "apply", "--reverse", "--index", "--directory=%s" % (paths.super_to_sub_relpath.decode("utf8"),), patch_abspath])
    p.communicate()
    if p.returncode != 0:
        # TODO explain how to recover!
        raise Exception("git failure")
    ...
    do_pop_push_update_metadata(paths, applied_index_new)
    with cwd(paths.super_abspath):
        superx.helper.add([paths.metadata_abspath])

    # TODO print commands for comitting and success messages
...
def cmd_push(args, parser):
    ...
    p = Popen(["git", "apply", "--index", "--directory=%s" % (paths.super_to_sub_relpath.decode("utf8"),), patch_abspath])
    p.communicate()
    if p.returncode != 0:
        # TODO explain how to recover!
        raise Exception("git failure")
    ...
    if applied_index_new + 1 == len(patches_dim.patches):
        ...
        do_pop_update_metadata_drop(paths)
    else:
        do_pop_push_update_metadata(paths, applied_index_new)
...
def read_metadata(path: bytes) -> Metadata:
    ...
    for metadata_line in metadata_lines:
        ...
        if metadata_line.line_type == LineType.KEY_VALUE:
            ...
            elif line_data.key == b"object-id":
...
def read_patches_dim(paths: Paths, metadata: Metadata) -> PatchesDim:
    try:
        patches = os.listdir(paths.patches_abspath)
...
def cmd_status(args, parser):
    ...
    for i, subproject in enumerate(subprojects):
        ...
        metadata = read_metadata(join(paths.super_abspath, subproject, b".subproject"))
        ...
        # TODO "paths_sub" is kind of guly here!
        paths_sub = do_paths(superx.path, subproject)
        patches_dim = read_patches_dim(paths_sub, metadata)
